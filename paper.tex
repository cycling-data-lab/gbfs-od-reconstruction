...
maintainers. Because the novelty of the contribution rests on \emph{not}
re-deriving the classical estimators, we first delimit what the existing two
literatures already settle.
...
\subsection{Partial pooling across networks is the only lever on \texorpdfstring{$\Tstar$}{T*}}
\label{sec:pooling}

For a single small network, \eqref{eq:Tstar} can run to years (Section~\ref{sec:audit}).
The only term the analyst controls is $\dim\theta$. Modelling the cost
hierarchically---a common deterrence law $\theta$ shared across the $N$ networks
with city-level random effects---divides the per-city parameter budget by the
number of poolable cities, and is therefore the mechanism that brings $\Tstar$
back from ``years'' to ``weeks''. The multi-system panel is thus
\emph{statistically load-bearing}, not merely a comparative convenience; a fixed
$\varepsilon$ imposed for cross-city comparability additionally caps the
achievable accuracy at $\delta_\OD\gtrsim\varepsilon$. With the law in hand, we
turn to measuring its constants on real feeds.
...
agencies and connect to the LOM/GDPR context.} Either way, the design choice
should be made knowingly, with the horizon law in view, rather than emerging by
accident from an identifier-rotation default.
